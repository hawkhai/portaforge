...
[18880] Failed to execute script 'pyi_rth_win32comgenpy' due to unhandled exception!
    ...
    def process_file(fpath, fname, ftype):
        ...
        # Windows 上 ZipFile 偶发 OSError 22（Invalid argument），短暂重试通常可恢复。
        _last_rc = None
        for _attempt in range(6):
            try:
                _last_rc = gzipfile(zipfile, to_version_path(runfile), fpath, ktouch=False)
                break
            except OSError:
                if _attempt + 1 >= 6:
                    raise
                time.sleep(0.08 * (_attempt + 1))
        assert 0 == _last_rc, _last_rc
